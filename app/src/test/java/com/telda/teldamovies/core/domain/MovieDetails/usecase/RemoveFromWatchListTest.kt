...
class RemoveFromWatchListTest {
    private lateinit var dao: WatchlistDao
    private lateinit var useCase: RemoveFromWatchList

    @Before
    fun setUp() {
        dao = mock(WatchlistDao::class.java)
        useCase = RemoveFromWatchList(dao)
    }

    @Test
    fun `invoke should remove movie from watchlist`() = runBlocking {
        ...
        useCase(movie)
        verify(dao).delete(movie.toWatchlistEntity())
    }
} 